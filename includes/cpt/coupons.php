<?php
class Obie_Events_Coupons_CPT
{
    public static $slug = 'event_coupon';
    ...
    public static function render_details_meta_box($post)
    {
        ...
        $code = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'coupon_code', true);
        $discount_type = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'discount_type', true) ?: 'fixed';
        $amount = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'amount', true) ?: '';
        $expiration_date = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'expiration_date', true) ?: '';
        $usage_limit = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'usage_limit', true) ?: '';
        $status = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'status', true) ?: 'active';
        $applicable_events = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'applicable_events', true) ?: array('all');
    ...
    {
        $total_uses = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'total_uses', true) ?: 0;
        $total_savings = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX .'total_savings', true) ?: 0;
        $last_used = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'last_used', true) ?: 0;
        $usage_history = get_post_meta($post->ID, OBIE_EVENTS_PLUGIN_PREFIX . 'usage_history', true) ?: array();
    ...
    public static function custom_column($column, $post_id)
    {
        switch ($column) {
            case 'code':
                $code = get_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'coupon_code', true);
                echo '<code>' . esc_html($code) . '</code>';
                break;

            case 'discount':
                $discount_type = get_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'discount_type', true) ?: 'fixed';
                $amount = get_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'amount', true) ?: 0;
            ...
            case 'usage':
                $total_uses = get_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'total_uses', true) ?: 0;
                $usage_limit = get_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'usage_limit', true);
                
                if (!empty($usage_limit)) {
                    echo esc_html($total_uses) . ' / ' . esc_html($usage_limit);
                } else {
                    echo esc_html($total_uses) . ' / ∞';
                }
                break;

            case 'expiration':
                $expiration_date = get_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'expiration_date', true);
            ...
            case 'status':
                $status = get_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'status', true) ?: 'active';
    ...
    public static function save_post($post_id)
    {
        ...
        if (!wp_verify_nonce($_POST['coupon_details_nonce'], 'coupon_details')) {
            return;
        }
        ...
        if (isset($_POST['coupon_code'])) {
            update_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'coupon_code', sanitize_text_field($_POST['coupon_code']));
        }

        if (isset($_POST['discount_type'])) {
            update_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'discount_type', sanitize_text_field($_POST['discount_type']));
        }

        if (isset($_POST['amount'])) {
            update_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'amount', (float) $_POST['amount']);
        }

        if (isset($_POST['expiration_date'])) {
            update_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'expiration_date', sanitize_text_field($_POST['expiration_date']));
        }

        if (isset($_POST['usage_limit'])) {
            update_post_meta($post_id, OBIE_EVENTS_PLUGIN_PREFIX . 'usage_limit', (int) $_POST['usage_limit']);
        }
